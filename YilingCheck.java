...
public class ZfbJYJZPayCheck {
    @Resource
    private CheckingQueryService checkingQueryService;
    @Resource
    private DataSourceResultsMapper dataSourceResultsMapper;
    private static final Logger log = LoggerFactory.getLogger(ZfbJYJZPayCheck.class);
    ...
    public Object execute(Map<String, Object> params) {
        ...
        log.info("[ZfbJYJZPayCheck-execute] checkingId={}", o.toString());
        ...
        for (int page = 0; page < 10000; page++) {
            ...
            log.info("[ZfbJYJZPayCheck-execute] dataSourceResults size={}", dataSourceResults.size());
            ...
            if (CollectionUtils.isEmpty(query)) {
                log.info("[ZfbJYJZPayCheck-execute] query is empty");
                errorIds.addAll(ids);
                continue;
            }
            ...
            for (DataSourceResults result : dataSourceResults) {
                ...
                checkingResults.add(
                        build(dataSourceId, checkingAggregate.getId(), checkResult, error, jsonObject, targetData));
            }
    ...
    public List<DataSourceResults> getDataSourceResults(Long dataSourceId, Object executeDate, Integer page) {
        ...
        if (executeDate != null) {
            log.info("[ZfbJYJZPayCheck-execute] dataSourceId={}, executeDate={}", dataSourceId, executeDate.toString());
            dataSourceResults = dataSourceResultsMapper.pageNoCheckResultByDate(dataSourceId, page * 1000, 1000,
                    Long.valueOf(executeDate.toString()));
        } else {
            log.info("[ZfbJYJZPayCheck-execute] dataSourceId={}, executeDate=null", dataSourceId);
            dataSourceResults = dataSourceResultsMapper.pageNoCheckResult(dataSourceId, page * 1000, 1000);
        }
        return dataSourceResults;
    }

    public CheckingResults build(Long dataSourceId, Long checkingId, boolean check, String error, JSONObject data,
            JSONObject data1) {
    ...
    public static String getSql(List<String> matchKeyValues) {
        String sql = "select channel_actual_amount as amount, third_serial, pay_num as orderNo from yt_pay.t_pay_sub where third_serial in (";
